...
class User(Base):
    ...
    def __repr__(self):
        return (f"User(name='{self.name}', age={self.age}, "
                f"gender='{self.gender}', nationality='{self.nationality}')")
...
class Posts(Base):
    ...
    liked_by_users: Mapped[List["User"]] = relationship( secondary=likes_table, back_populates='liked_posts', )
    ...
    def __repr__(self):
        return (f"Post(title='{self.title}', "
                f"description='{self.description}', user={self.user.name})")
...
class Comments(Base):
    ...
    def __repr__(self):
        return (f"Comment(user_id={self.user_id}, "
                f"post_id={self.post_id}, comment='{self.comment}')")
